...
depletion_estimation_function <- function(dir,seq,par_name,cores,its){
  ...
  rec_dev_mat <- gen_rec_devs(years = 101,its = 100,sigma_r = 0.4,bias_adjust = TRUE,seed = 1916)
  
  nat_m <- seq
  df <- do.call("rbind", replicate( 
    length(nat_m), sim_df[sim_df$ce.par_name == par_name,], simplify = FALSE)) 
  ...
  df$scenarios <- paste0("par_val_",nat_m)
  df$error_type <- NULL
  ...
  run_ss3sim(iterations = 1:its,simdf = df,parallel = TRUE,parallel_iterations = TRUE,
             user_recdevs = rec_dev_mat)
  
  saveRDS(df,file = paste0(dir,"/",df,".rdata"))
  parSapply(cl,
            X = df$scenarios,
